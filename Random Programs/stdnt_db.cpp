...
int main()
{
    ...
    ofstream outF("files\\students_db.mpd",ios::app|ios::out);
    ifstream inF("files\\students_db.mpd");
    ...
    case 50 :
        {
            system("CLS");
            cout << "///// Record Display /////" << "\t\t\tEntries Yet: " << roll.size() << endl << endl;
    ...
    case 51 :
        {
            system("CLS");
            cout << "///// Record Replace /////" << "\t\t\tEntries Yet: " << roll.size() << endl << endl;
            cout << "Enter record number: ";
            cin >> choice;
            while( (choice<0) )
            {
    ...
    flag = 1;
        ...
        ofstream dispF("students_database.txt");
        dispF << "***************** Student's Database File *****************" << endl << endl << endl;
        dispF << left << setw(11) << "Rec No." << setw(12) << "Roll No." << setw(36) << "Name" << right << setw(4) << "CGPA" << endl << endl;
        for(int di=0; di<roll.size(); ++di)
        {
            dispF << left << setw(11) << di << setw(12) << roll[di] << setw(36) << name[di] << setw(4) << right << fixed << setprecision(2) << cgp[di] << endl;
        }
...
{
    outFile.close();
    outFile.open("files\\students_db.mpd");
    for(int i=0; i<roll.size(); ++i)
    {
        outFile << name[i] << endl << roll[i] << endl << cgp[i] << endl;
    }
    outFile.close();
    outFile.open("files\\students_db.mpd",ios_base::app);
}
...
void Delay(double milSec)
{
    ...
    while( time(NULL) < (current_time + milSec) )
    {
